...
def find_target(
    ...
    for i, (resources, *_) in enumerate(curr_state):
        if resources > 0:
            score = resources // time_to_harvest(distances[i], resources, total_my_ants)
            if field.cells[i].is_egg():
                score -= -1 + i_move * 3 / expected_length
            if score > best_score:
                best_score = score
                best_target = i
    return best_target
...
while True:
    ...
    #     print(f"{dist_to_resources = }", file=sys.stderr, flush=True)
    #
